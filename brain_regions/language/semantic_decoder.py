...
class SemanticDecoder(BrainRegion):
    """Decode semantic meaning from text and neural representations"""

    def __init__(self, event_bus: EventBus, model_name: str = "all-MiniLM-L6-v2"):
    ...
    async def initialize(self):
        ...
        logger.info("initializing_semantic_decoder")

        # Pre-compute embeddings for common concepts
    ...
    async def _initialize_concept_space(self):
        """Initialize embeddings for common concepts"""
        ...
        for concept in concepts:
            self.concept_embeddings[concept] = self.encoder.encode(concept)
    ...
    def _find_nearest_concepts(self, vector: np.ndarray, k: int = 5) -> List[Tuple[str, float]]:
        ...
        for concept, concept_vec in self.concept_embeddings.items():
            ...
            similarity = np.dot(vector, concept_vec) / (
                    np.linalg.norm(vector) * np.linalg.norm(concept_vec)
            )
